...
const wrapValue = R.ifElse(isFunction, R.identity, v => () => v);
...
function Interceptor(handler) {
  ...
  this.onCall = R.curry((index, value) => {
    ...
    return Interceptor.of(call =>
      call.index === index ? Maybe.Just(wrappedValue.apply(this, call.args)) : handler(call)
    );
  });
  ...
  this.withArgs = R.curry((args, value) => {
    ...
    const matchingValue = call => Maybe.Just(wrappedValue.apply(this, call.args));
